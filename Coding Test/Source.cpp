...
1. 로컬 스페이스(3D 모델 구축)
2. 월드 스페이스(가상 공간 배치)
3. 정점 단위의 음영 계산(그림자 처리)
4. 뷰 스페이스(카메라 공간의 전개)
5. 컬링(렌더링 연산 제외 처리)
6. 조명(3D 공간의 빛을 구현)
7. 클리핑(카메라 시야에 사라진 폴리곤 비렌더링 처리)
8. 투영(3D 물체를 2D 평면에 표현하기 위해 3D 모델 좌표를 2D 평변 좌표로 변환함)
9. 뷰포트 변환(윈도우 좌표계 변환)
10. 폴리곤 셋업(정점 파이프라인 결과 → 픽셀 단위 분해 → 픽셀 파이프라인)
11. 래스터 라이즈(폴리곤 출력에 필요한 픽셀 컬러를 계산)
12. 픽셀 단위 음영 계산(픽셀 쉐이더로 음영 계산)
13. 맵핑(텍스쳐 적용)
14. 렌더 백엔드(과정 검증)
15. 출력(최종)